...
export class GrapheListeAdjacence {
  ...
  ajouterSommet() {
      this.sommets.push(new Sommet((Number(this.sommets[this.sommets.length - 1].nom) + 1).toString()));
  }
  ...
  retirerSommet() {
    ...
    getPredecesseurs(sommetSource: Sommet) {
        ...
        for(const sommetGraphe of this.sommets) {
            const estPredecesseur = sommetGraphe.sommetArc.some(sommet => sommet.nom === sommetSource.nom);
            if(estPredecesseur) {
    ...
    getVoisins(sommetSource: Sommet) {
        return [...new Set([...this.getPredecesseurs(sommetSource), ...this.getSuccesseurs(sommetSource)])]; // Set pour éliminer les doublons
    }
